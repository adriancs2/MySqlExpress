...
namespace System.handlers
{
    ...
    {
        public static void List(Ctx ctx)
        {
            ...
            int year;
            if (!int.TryParse((ctx.Request.QueryString["year"] ?? "").Trim(), out year)) year = 2024;
        ...
        public static void Assign(Ctx ctx)
        {
            if (!RequireConn(ctx)) return;

            var f = ctx.Request.Form;
            int year;     int.TryParse((f["year"]     ?? "").Trim(), out year);
            int playerId; int.TryParse((f["playerId"] ?? "").Trim(), out playerId);
            int teamId;   int.TryParse((f["teamId"]   ?? "").Trim(), out teamId);
            int level;    int.TryParse((f["level"]    ?? "1").Trim(), out level);

            decimal score;
            if (!decimal.TryParse((f["score"] ?? "0").Trim(), out score))
                score = 0m;
