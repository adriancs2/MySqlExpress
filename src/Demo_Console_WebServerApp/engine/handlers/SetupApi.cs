...
namespace System.handlers
{
    ...
    ///
    /// Each method writes a JSON response into the per-request <see cref="Ctx"/>.
    /// The dispatcher in <see cref="Program"/> turns that buffer into bytes
    /// and sends it down the socket — same shape as the ASP.NET version,
    /// just without the ambient <c>HttpContext.Current</c>.
    /// </summary>
    public static class SetupApi
    {
        // ---------- connection string: save / test / clear ----------

        public static void SaveConnString(Ctx ctx)
        {
            string connStr = (ctx.Request.Form["connStr"] ?? "").Trim();
        ...
        public static void TestConnString(Ctx ctx)
        {
            string connStr = (ctx.Request.Form["connStr"] ?? "").Trim();
            if (string.IsNullOrEmpty(connStr))
            {
        ...
        public static void ClearConnString(Ctx ctx)
        {
            Config.Clear();
        ...
        public static void CreateTables(Ctx ctx)
        {
        ...
        public static void DropTables(Ctx ctx)
        {
            if (!RequireConnOrEnd(ctx)) return;
        ...
        public static void SeedSampleData(Ctx ctx)
        {
            if (!RequireConnOrEnd(ctx)) return;
        ...
        static bool RequireConnOrEnd(Ctx ctx)
        {
