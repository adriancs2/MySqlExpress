# MySqlExpress Demo — Pageless ASP.NET Web Forms (Scoped edition)
...
3. **Show how tiny, well-chosen wrappers pay for themselves.** Two small helpers — `Db.Run(m => ...)` on the server (52 lines) and `apiPostForm(url, data, cb)` in the browser (~30 lines) — collapse the repeating lifecycle code so every handler reads as pure business logic. Connection management, command disposal, fetch boilerplate, JSON parsing: all wrapped, all invisible, all correct.
...
| --- | --- | --- |
| **[Flat](../Demo_ASPNET_Pageless)** | Transparency. Every `using` block, every `fetch`, every `try`/`catch` is written out at the call-site. Nothing is hidden. | First. Read this to see what Pageless Architecture actually *does*. |
| **Scoped** *(this project)* | Economy. A tiny `Db.Run(m => ...)` wrapper and a thin `apiPostForm(...)` helper eliminate ~80% of the per-handler boilerplate. | Second. Read this to see how a production codebase trims the noise without losing the architecture. |
...
> **Reader note.** If you haven't seen the Flat edition yet, read it first. The Scoped edition makes a lot more sense once you've seen the boilerplate it removes.
...
```
1. Clone the repo.
2. Open Demo_ASPNET_Pageless_Scoped.csproj in Visual Studio (2019 or newer).
...
### The Db wrapper (the whole thing)

`engine/Db.cs` is 52 lines. It's the only piece of infrastructure that separates this edition from Flat:
...
`Db.Run` for writes and commands that return nothing useful. `Db.Get<T>` for anything that produces a value — a row, a list, a scalar. Open the connection, hand `MySqlExpress` to the caller's lambda, dispose on the way out. That's the whole thing. No pooling layer, no unit-of-work pattern, no abstractions stacked on top.
...
One static class per page. `SiteTemplate.Header()` / `Footer()` is the layout. `$@"..."` is the view engine. The handler body is pure business logic — SQL and markup, nothing else. Connection lifecycle is a solved problem handled by the one helper file you only ever need to read once.

Compare with the [Flat edition's version of this same handler](../Demo_ASPNET_Pageless/engine/handlers/PlayerList.cs) — identical behaviour, but the `using` chain is written out inline. Neither is wrong. They're answering different questions.
...
The browser-side JS (`js/Site.js`) has a `window.apiPostForm(url, payload, onSuccess, onError)` helper that posts form-encoded bodies and expects JSON back:
...
Three arguments: the URL, the payload, the success callback. A fourth optional `onError` callback exists for network-level failures; most call-sites omit it because the helper logs the error to the console and that's enough for anything short of offline-mode UX. No framework, no build step.

### Two error channels

The wrapped helpers preserve the same two-channel error model as the Flat edition — the abstraction doesn't collapse it, just moves it inside the helper:

- **HTTP 200 + `{ success: false, message: "..." }`** — business-level failure (validation, duplicate, SQL threw). The success callback reads `d.success` and flashes.
- **`.catch` inside `apiPostForm`** — transport failure (network drop, CORS, server 500). Logged to console; optional `onError` callback fires if provided.

The two channels are separate on purpose. "Connection string is wrong" is a business-level `success: false` (the server ran fine and reported a user-level problem); "the server crashed" is a transport-level catch. Mixing them is what makes error handling in hand-rolled apps quietly painful.
...
├── js/
│   └── Site.js                   (apiGet, apiPostForm, escapeHtml, flash, sidebar toggle)
├── fonts/
...
└── engine/
    ...
    ├── Db.cs                     (Db.Run / Db.Get — the 52-line scoped wrapper)
...
- **`engine/` (not `handlers/`)** — shared infrastructure. Two files earn their keep here: `Db.cs` wraps the connection lifecycle, `Site.js` wraps the fetch lifecycle. Everything else is the same as the Flat edition.
- **`engine/models/`** — POCOs. Field and property names match MySQL column names.
- **`engine/handlers/`** — one static class per route. Each handler holds its SQL, its HTML, and its JS inline — but with the request-lifecycle noise lifted out.
...
If the page needs a JSON endpoint, create `MyPageApi.cs` next to it and route `/api/mypage/something` to the appropriate method. That's the whole extension model. Inside the handler, call `Db.Run(m => ...)` or `Db.Get(m => ...)` to get a ready `MySqlExpress` instance — don't open your own connections unless you have a reason to.
...
- **Sibling demo:** [Demo_ASPNET_Pageless (Flat edition)](../Demo_ASPNET_Pageless) — the same app with every `using` block and every `fetch` written out at the call-site.
