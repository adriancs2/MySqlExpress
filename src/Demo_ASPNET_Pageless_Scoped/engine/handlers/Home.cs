...
<script>
function testConn() {
    var s = document.getElementById('connStr').value;
    window.clearFlash();
    window.apiPostForm('/api/setup/test-conn', { connStr: s }, function (d) {
...
function saveConn() {
    var s = document.getElementById('connStr').value;
    window.clearFlash();
    window.apiPostForm('/api/setup/save-conn', { connStr: s }, function (d) {
...
function createTables() {
    window.clearFlash();
    window.apiPostForm('/api/setup/create-tables', {}, function (d) {
...
function seedData() {
    window.clearFlash();
    window.apiPostForm('/api/setup/seed', {}, function (d) {
        window.flash(d.success ? 'success' : 'error', d.message);
        if (d.success) setTimeout(function() { window.location.reload(); }, 700);
    });
}
function dropTables() {
    ...
    window.clearFlash();
    window.apiPostForm('/api/setup/drop-tables', {}, function (d) {
        window.flash(d.success ? 'success' : 'error', d.message);
        if (d.success) setTimeout(function() { window.location.reload(); }, 700);
    });
}
function clearConn() {
    ...
    window.apiPostForm('/api/setup/clear-conn', {}, function (d) {
        if (d.success) window.location.reload();
    });
}
</script>
