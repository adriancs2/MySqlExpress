...
namespace Demo_ASPNET_Core.engine.handlers
{
    ...
    {
        public static void Run(HttpContext ctx)
        {
            ...
            string sql = (form["sql"].ToString() ?? "").Trim();
            if (sql.Length == 0)
            {
                ApiHelper.WriteError(ctx, "SQL is empty.");
                return;
            }
                ...
                if (isQuery)
                {
                    ...
                    foreach (DataRow dr in dt.Rows)
                    {
                        ...
                        foreach (DataColumn dc in dt.Columns)
                        {
                            ...
                            // byte[] (BLOB) doesn't serialize meaningfully — show length instead.
                            if (v is byte[]) v = $"<byte[{((byte[])v).Length}]>";
                            row[dc.ColumnName] = v;
                        }
                        rowsOut.Add(row);
                    }
            ...
            {
                ApiHelper.WriteError(ctx, "SQL error: " + ex.Message);
            }
        }
