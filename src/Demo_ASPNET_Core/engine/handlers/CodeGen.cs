...
namespace Demo_ASPNET_Core.engine.handlers
{
    ...
    {
        public static void HandleRequest(HttpContext ctx)
        {
            if (!Config.HasConnString) { Render.NotConfigured(ctx); return; }
            ...
            {
                Render.Error(ctx, "Code Generation", "codegen", ex.Message);
                return;
            }
...
<div class='card'>
    ...
    <div class='form-row'>
        ...
        <div class='form-field' style='max-width:300px'>
            ...
            <select id='cgStyle'>
                <option value='PrivateFieldsPublicProperties' selected>Private fields + public properties</option>
...
async function runGen(kind) {
    ...
    formData.append('table', document.getElementById('cgTable').value);
    formData.append('style', document.getElementById('cgStyle').value);
    formData.append('sql',   document.getElementById('cgSql').value);

    var el = document.getElementById('cgResult');

    try {
        const response = await fetch('/api/codegen/run', {
            method: 'POST',
            body: formData
        });
        ...
        el.innerHTML =
            ""<div class='card'><div class='flash flash-error'><i class='fas fa-exclamation-triangle'></i> Network error.</div></div>"";
    }
}
