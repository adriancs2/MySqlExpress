...
namespace Demo_ASPNET_Core.engine.handlers
{
    ...
    public static class PlayerEdit
    {
        public static void HandleRequest(HttpContext ctx)
        {
            ...
            {
                Render.NotConfigured(ctx); return;
            }

            string path = (ctx.Request.Path.Value ?? "").ToLowerInvariant().Trim();
            ...
            if (id > 0)
            {
                ...
                {
                    Render.Error(ctx, "Edit Player", "players", ex.Message);
                    return;
                }

                if (p == null || p.Id == 0)
                {
                    Render.Error(ctx, "Edit Player", "players", "Player not found.");
                    return;
                }
            }
...
async function savePlayer() {
    ...
    formData.append('dateRegister', document.getElementById('dateRegister').value);
