...
");
            sb.Append(@"
<script>
function assignRoster(year) {
    window.clearFlash();
    var payload = {
        year:     year,
        playerId: document.getElementById('rosterPlayer').value,
        teamId:   document.getElementById('rosterTeam').value,
        score:    document.getElementById('rosterScore').value,
        level:    document.getElementById('rosterLevel').value
    };
    window.apiPostForm('/api/roster/assign', payload, function (d) {
        if (d.success) window.location.reload();
        else window.flash('error', d.message || 'Assign failed');
    });
}
function removeRoster(year, playerId) {
    if (!confirm('Remove this roster entry?')) return;
    window.apiPostForm('/api/roster/delete', { year: year, playerId: playerId }, function (d) {
        if (d.success) window.location.reload();
